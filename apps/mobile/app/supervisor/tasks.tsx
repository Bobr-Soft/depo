...
import { ArrowLeft, ClipboardList, RefreshCw, AlertCircle, AlertTriangle } from "@tamagui/lucide-icons";
...
export default function SupervisorTasksScreen() {
  ...
  const handleChangePriority = (task: TaskComplete) => {
    Alert.alert(
      ...
      `${task.name}\nJelenlegi: ${PRIORITY_LABELS[task.priority] ?? String(task.priority)}`,
      [
    ...
    <YStack flex={1} backgroundColor="$background" style={{ paddingTop: insets.top + 16 }}>
      <YStack paddingHorizontal="$4" gap="$3" marginBottom="$2">
        <XStack alignItems="center" gap="$3">
          ...
          <YStack flex={1}>
            <H2 color="$color12">Feladatsor</H2>
            <Text fontSize={14} color="$color10">Aktív és várakozó feladatok</Text>
          </YStack>
          ...
          <YStack gap="$3">
            <XStack gap="$3" marginBottom="$2">
              <Card flex={1} padding="$3" borderRadius="$4" backgroundColor="$color3" alignItems="center" gap="$1" borderWidth={1} borderColor="$color4">
                <Text fontSize={20} fontWeight="800" color="$blue10">{pendingCount}</Text>
                <Text fontSize={11} color="$color10" textAlign="center">Várakozik</Text>
              </Card>
              <Card flex={1} padding="$3" borderRadius="$4" backgroundColor="$color3" alignItems="center" gap="$1" borderWidth={1} borderColor="$color4">
                <Text fontSize={20} fontWeight="800" color="$orange10">{inProgressCount}</Text>
                <Text fontSize={11} color="$color10" textAlign="center">Folyamatban</Text>
              </Card>
              <Card flex={1} padding="$3" borderRadius="$4" backgroundColor="$color3" alignItems="center" gap="$1" borderWidth={1} borderColor="$color4">
                <Text fontSize={20} fontWeight="800" color="$color12">{tasks.length}</Text>
              ...
              >
                <XStack alignItems="center" gap="$3">
                  <YStack width={40} height={40} backgroundColor="$blue5" borderRadius="$3" alignItems="center" justifyContent="center">
                    <ClipboardList size={18} color="$blue10" />
                  </YStack>
                  <YStack flex={1} gap="$0.5">
                    ...
                    <Text fontSize={12} color="$color10">
                      {task.assigned_user_data?.email ?? "Nincs hozzárendelve"} · {task.items.length} tétel
                    </Text>
                  </YStack>
                  <Button
                    size="$2"
                    ...
                    disabled={!syncStatus.isOnline || task.status === "completed" || task.status === "cancelled"}
                  >
                    P{task.priority}
                  </Button>
                </XStack>
