...
function defaultQueryMock(sql, params) {
  // Auth middleware: return matching user row with correct role
  if (typeof sql === 'string' && sql.includes('FROM users WHERE')) {
    const email = (params && params[0]) || '';
    if (email.toLowerCase().includes('worker') || email.toLowerCase().includes('unrelated')) {
      return Promise.resolve([[{ id: 1, email, role: 'worker' }], []]);
    }
    return Promise.resolve([[{ id: 1, email, role: 'admin' }], []]);
  }
